...
class _TechGamingPageState extends State<TechGamingPage> with SingleTickerProviderStateMixin {
        ...
        AnimatedBuilder(
          ...
          builder: (_, __) => Container(
            ...
            child: Row(children: [
              ...
              const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Tech × Gaming', style: TextStyle(color:Colors.white,fontSize:24,fontWeight:FontWeight.w900)),SizedBox(height:5),Text('أخبار ومراجعات وبرمجة وألعاب في مكان واحد',style:TextStyle(color:Colors.white70))])),
            ]),
          ),
        ),
        const SizedBox(height:16),
        SegmentedButton<int>(segments: const [ButtonSegment(value:0,label:Text('التقنية'),icon:Icon(Icons.memory_rounded)),ButtonSegment(value:1,label:Text('الألعاب'),icon:Icon(Icons.sports_esports_rounded)),ButtonSegment(value:2,label:Text('الإنجازات'),icon:Icon(Icons.emoji_events_rounded))], selected:{section}, onSelectionChanged:(s)=>setState(()=>section=s.first)),
        const SizedBox(height:16),
        if (section == 0) _techView() else if (section == 1) _gamesView() else _achievementsView(),
        const SizedBox(height:14),
        GlassCard(child: Row(children: [const Icon(Icons.security_rounded,color:SN.cyan),const SizedBox(width:10),Expanded(child:Text(safeMode?'فلترة الأمان مفعّلة للمحتوى المناسب للعمر.':'فلترة الأمان متوقفة.'))])),
      ],),
    );
  }
  ...
  Widget _achievementsView() => Column(crossAxisAlignment:CrossAxisAlignment.start,children:[const Text('إنجازات التقنية والألعاب',style:TextStyle(fontSize:20,fontWeight:FontWeight.w900)),const SizedBox(height:10),...achievements.map((a)=>Padding(padding:const EdgeInsets.only(bottom:10),child:GlassCard(child:Row(children:[Container(width:54,height:54,decoration:BoxDecoration(shape:BoxShape.circle,color:a.color.withValues(alpha:.12)),child:Icon(a.icon,color:a.color)),const SizedBox(width:12),Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(a.title,style:const TextStyle(fontWeight:FontWeight.w900,fontSize:16)),const SizedBox(height:3),Text(a.subtitle,style:const TextStyle(color:SN.textSec,fontSize:13)),const SizedBox(height:8),const LinearProgressIndicator(value:.35,minHeight:6)])),const SizedBox(width:4),const Icon(Icons.lock_outline_rounded,color:SN.textMut)])))),GlassCard(child:Row(children:[const Icon(Icons.privacy_tip_outlined,color:SN.cyan),const SizedBox(width:10),Expanded(child:Text('الأوسمة تُكتسب من النشاط الفعلي داخل التطبيق.'))]))]);
  Widget _contentCard(String category,String title,IconData icon,String description)=>Padding(padding:const EdgeInsets.only(bottom:10),child:GlassCard(child:Row(children:[Container(width:52,height:52,decoration:BoxDecoration(borderRadius:BorderRadius.circular(16),color:SN.violet.withValues(alpha:.10)),child:Icon(icon,color:SN.violet,size:28)),const SizedBox(width:12),Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Row(children:[Text(category,style:const TextStyle(color:SN.cyan,fontSize:12,fontWeight:FontWeight.w800)),const Spacer(),const Icon(Icons.chevron_left_rounded,color:SN.textMut)]),const SizedBox(height:3),Text(title,style:const TextStyle(fontSize:16,fontWeight:FontWeight.w900)),const SizedBox(height:3),Text(description,style:const TextStyle(color:SN.textSec,height:1.3))]))])));
  Widget _featureTile(IconData icon,String title,String text)=>Padding(padding:const EdgeInsets.only(bottom:10),child:GlassCard(child:ListTile(contentPadding:EdgeInsets.zero,leading:Icon(icon,color:SN.cyan,size:30),title:Text(title,style:const TextStyle(fontWeight:FontWeight.w900)),subtitle:Text(text))));
}
