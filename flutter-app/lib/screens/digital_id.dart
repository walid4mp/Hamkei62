...
class _DigitalIdPageState extends State<DigitalIdPage> {
        ...
        builder: (context, snap) {
          ...
          return ListView(
            ...
            children: [
              ...
              Text('البيانات تُسحب تلقائيًا من حسابك. يمكنك التحكم في ظهور تاريخ الميلاد والجنس واسم المجموعة من تخصيص البطاقة.', textAlign: TextAlign.center, style: TextStyle(fontSize: 12, color: SN.textSec)),
            ],
          );
        },
      ),
      ...
      child: Container(
        ...
        child: Column(children: [
          Row(children: [
            ...
            Expanded(child: Text(L10n.t('البطاقة الرقمية'), style: TextStyle(fontSize: 23, fontWeight: FontWeight.w900))),
            Icon(verified ? Icons.verified_rounded : Icons.verified_outlined, color: verified ? SN.green : SN.textMut, size: 42),
          ]),
          const SizedBox(height: 4),
          Text('SocialNova', style: TextStyle(color: SN.cyan, fontSize: 14, fontWeight: FontWeight.w800)),
          const SizedBox(height: 14),
          Stack(alignment: Alignment.center, children: [
            ...
            CircleAvatar(radius: 78, backgroundColor: SN.bg3, backgroundImage: avatar.isNotEmpty ? NetworkImage(avatar) : null, child: avatar.isEmpty ? Icon(Icons.person_rounded, size: 72) : null),
          ]),
          const SizedBox(height: 12),
          Text(name, textAlign: TextAlign.center, style: const TextStyle(fontSize: 27, fontWeight: FontWeight.w900)),
          Text('@$username', style: TextStyle(fontSize: 16, color: SN.textSec)),
          const SizedBox(height: 10),
          Container(padding: EdgeInsets.symmetric(horizontal: 14, vertical: 7), decoration: BoxDecoration(color: (verified ? SN.green : a).withValues(alpha: .12), borderRadius: BorderRadius.circular(30), border: Border.all(color: (verified ? SN.green : a).withValues(alpha: .35))), child: Text(verified ? 'حساب موثق • ${d['verificationTier'] ?? 'NORMAL'}' : 'حساب SocialNova', style: TextStyle(color: verified ? SN.green : a, fontWeight: FontWeight.w800))),
          const SizedBox(height: 14),
          ...
          const SizedBox(height: 12),
          Text('بطاقة ملفك في SocialNova', style: TextStyle(color: SN.textSec, fontSize: 12)),
        ]),
      ),
    );
  }

  Widget _info(String label, String value, IconData icon, Color a) => Container(width: 154, constraints: BoxConstraints(minHeight: 76), padding: EdgeInsets.all(9), decoration: BoxDecoration(color: Colors.black.withValues(alpha: .28), borderRadius: BorderRadius.circular(14), border: Border.all(color: a.withValues(alpha: .22))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Icon(icon, size: 20, color: a), SizedBox(height: 4), Text(label, style: TextStyle(fontSize: 10, color: SN.textSec)), SizedBox(height: 2), Text(value, maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800))]));

  Widget _statBox(String value, String label, IconData icon, Color a) => Container(width: 145, padding: EdgeInsets.all(10), decoration: BoxDecoration(color: Colors.black.withValues(alpha: .28), borderRadius: BorderRadius.circular(14), border: Border.all(color: a.withValues(alpha: .22))), child: Row(children: [Icon(icon, color: a, size: 22), SizedBox(width: 8), Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(value, style: TextStyle(fontSize: 17, fontWeight: FontWeight.w900)), Text(label, style: TextStyle(fontSize: 10, color: SN.textSec))])]));

  Widget _mini(String value, String label) => Column(children: [Text(value, style: TextStyle(fontWeight: FontWeight.w900)), Text(label, style: TextStyle(fontSize: 9, color: SN.textSec))]);
  ...
  Future<void> _settings() async {
    ...
    await showModalBottomSheet(context: context, isScrollControlled: true, backgroundColor: SN.bg2, builder: (ctx) => StatefulBuilder(builder: (ctx, updateLocal) => Padding(padding: EdgeInsets.fromLTRB(16, 18, 16, 28 + MediaQuery.viewInsetsOf(ctx).bottom), child: ListView(shrinkWrap: true, children: [
      const Text('تخصيص البطاقة الرقمية', style: TextStyle(fontSize: 21, fontWeight: FontWeight.w900)),
      const SizedBox(height: 6),
      Text('اختر المظهر والمعلومات التي تريد إظهارها.', style: TextStyle(color: SN.textSec)),
      const SizedBox(height: 14),
      const Text('النمط', style: TextStyle(fontWeight: FontWeight.w800)),
      const SizedBox(height: 8),
      Wrap(spacing: 8, children: ['midnight', 'gold', 'green', 'violet'].map((x) => ChoiceChip(label: Text(x), selected: localTheme == x, onSelected: (_) => updateLocal(() => localTheme = x))).toList()),
      const SizedBox(height: 12),
      DropdownButtonFormField<String>(value: localVisibility, decoration: InputDecoration(labelText: 'ظهور البطاقة'), items: [DropdownMenuItem(value: 'PUBLIC', child: Text(L10n.t('الجميع'))), DropdownMenuItem(value: 'FRIENDS', child: Text(L10n.t('الأصدقاء'))), DropdownMenuItem(value: 'PRIVATE', child: Text(L10n.t('أنا فقط')))], onChanged: (x) { if (x != null) updateLocal(() => localVisibility = x); }),
      const SizedBox(height: 10),
      DropdownButtonFormField<String?>(value: localGroup, decoration: InputDecoration(labelText: 'اسم المجموعة'), items: [DropdownMenuItem<String?>(value: null, child: Text(L10n.t('بدون مجموعة'))), ...myGroups.whereType<Map>().map((g) => DropdownMenuItem<String?>(value: '${g['id']}', child: Text('${g['name'] ?? ''}', overflow: TextOverflow.ellipsis)))], onChanged: (x) => updateLocal(() => localGroup = x)),
      SwitchListTile(value: localGender, onChanged: (x) => updateLocal(() => localGender = x), title: const Text('إظهار الجنس'), subtitle: const Text('يظهر فقط إذا كانت بيانات الجنس محفوظة في الحساب.')),
      SwitchListTile(value: localBirth, onChanged: (x) => updateLocal(() => localBirth = x), title: const Text('إظهار تاريخ الميلاد'), subtitle: const Text('يظهر التاريخ الكامل عند تفعيل هذا الخيار.')),
      SwitchListTile(value: localFollowers, onChanged: (x) => updateLocal(() => localFollowers = x), title: const Text('عدد المتابعين')),
      SwitchListTile(value: localPosts, onChanged: (x) => updateLocal(() => localPosts = x), title: const Text('عدد المنشورات')),
      SwitchListTile(value: localStories, onChanged: (x) => updateLocal(() => localStories = x), title: const Text('عدد القصص')),
      SwitchListTile(value: localActivity, onChanged: (x) => updateLocal(() => localActivity = x), title: const Text('إحصائيات النشاط')),
      const SizedBox(height: 8),
      FilledButton.icon(onPressed: () async { theme = localTheme; shape = localShape; visibility = localVisibility; groupId = localGroup; showGender = localGender; showBirthDate = localBirth; showFollowers = localFollowers; showPosts = localPosts; showStories = localStories; showActivity = localActivity; Navigator.of(context).pop(); await save(); if (mounted) setState(() => future = _load()); }, icon: const Icon(Icons.save_rounded), label: const Text('حفظ التخصيص')),
    ]))));
  }
}
