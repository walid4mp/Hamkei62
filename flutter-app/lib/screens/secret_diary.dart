...
class _SecretDiaryPageState extends State<SecretDiaryPage> {
  ...
  Future<void> _unlock() async {final pin=await storage.read(key:pinKey);if(pin==null||pin.isEmpty){setState(()=>locked=false);return;}final c=TextEditingController();final v=await showDialog<String>(context:context,builder:(ctx)=>AlertDialog(title:Text('فتح المذكرات السرية'),content:TextField(controller:c,obscureText:true,keyboardType:TextInputType.number,maxLength:8,decoration:InputDecoration(labelText:'الرمز السري')),actions:[TextButton(onPressed:()=>Navigator.pop(ctx),child:Text('إلغاء')),FilledButton(onPressed:()=>Navigator.pop(ctx,c.text.trim()),child:Text('فتح'))]));if(v==pin){setState(()=>locked=false);}else if(v!=null&&mounted){ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text(L10n.t('الرمز غير صحيح'))));}}
  Future<void> _setPin() async {final c=TextEditingController();final v=await showDialog<String>(context:context,builder:(ctx)=>AlertDialog(title:Text('تعيين رمز المذكرات'),content:TextField(controller:c,obscureText:true,keyboardType:TextInputType.number,maxLength:8,decoration:InputDecoration(labelText:'4 إلى 8 أرقام')),actions:[TextButton(onPressed:()=>Navigator.pop(ctx),child:Text('إلغاء')),FilledButton(onPressed:()=>Navigator.pop(ctx,c.text.trim()),child:Text('حفظ'))]));if(v==null)return;if(v.length<4){if(mounted)ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text(L10n.t('استخدم 4 أرقام على الأقل'))));return;}await storage.write(key:pinKey,value:v);if(mounted)setState(()=>locked=true);}
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, set) => Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            const SizedBox(height: 8),
            DropdownButtonFormField<int>(value: days, decoration: const InputDecoration(labelText: 'الحذف التلقائي'), items: [
              DropdownMenuItem(value: 0, child: Text(L10n.t('لا يوجد'))), DropdownMenuItem(value: 1, child: Text(L10n.t('بعد يوم'))),
              DropdownMenuItem(value: 7, child: Text(L10n.t('بعد 7 أيام'))), DropdownMenuItem(value: 30, child: Text(L10n.t('بعد 30 يومًا'))),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: entries.isEmpty
          ? Center(child: Text(L10n.t('لا توجد مذكرات بعد')))
          : ListView.separated(
