...
class _AchievementsPageState extends State<AchievementsPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _shareAchievement(AchievementItem a) async {
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(L10n.t('تم تجهيز الإنجاز للمشاركة في قصتك'))));
  }
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.fromLTRB(16, 8, 16, 110), children: [
        ...
        SegmentedButton<int>(segments: [ButtonSegment(value: 0, label: Text(L10n.t('الكل')), icon: const Icon(Icons.grid_view_rounded)), ButtonSegment(value: 1, label: Text(L10n.t('مكتملة')), icon: const Icon(Icons.verified_rounded)), ButtonSegment(value: 2, label: Text(L10n.t('موسمية')), icon: const Icon(Icons.auto_awesome_rounded))], selected: {tab}, onSelectionChanged: (s) => setState(() => tab = s.first)),
        const SizedBox(height: 16),
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(L10n.t('إنجازاتك'), style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w900)), Text('${filtered.length} بطاقة', style: TextStyle(color: SN.textMut))]),
  ...
  Widget _card(AchievementItem a) => Padding(padding: const EdgeInsets.only(bottom: 10), child: GlassCard(padding: const EdgeInsets.all(14), child: Row(children: [
    ...
    SizedBox(width: 13), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [Expanded(child: Text(a.title, style: TextStyle(fontWeight: FontWeight.w900, fontSize: 16))), if (a.seasonal) Chip(label: Text(L10n.t('موسمي')), visualDensity: VisualDensity.compact)]), SizedBox(height: 3), Text(a.subtitle, style: TextStyle(color: SN.textSec, fontSize: 13)), SizedBox(height: 9), ClipRRect(borderRadius: BorderRadius.circular(8), child: LinearProgressIndicator(value: a.ratio, minHeight: 6, backgroundColor: SN.strokeSoft, valueColor: AlwaysStoppedAnimation(a.color))), SizedBox(height: 4), Text('${a.progress}/${a.target}', style: TextStyle(color: SN.textMut, fontSize: 11))])),
