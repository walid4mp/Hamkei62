...
class _FitnessChallengesPageState extends State<FitnessChallengesPage> {
        ...
        children: [
          GlassCard(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Container(width: 52, height: 52, decoration: BoxDecoration(color: SN.cyan.withValues(alpha: .12), shape: BoxShape.circle), child: Icon(Icons.sports_rounded, color: SN.cyan, size: 28)),
                const SizedBox(width: 12),
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(L10n.t('تحرّك بطريقتك 🏃'), style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900)), SizedBox(height: 4), Text(L10n.t('تحديات صحية خفيفة، سجّل نشاطك وتابع تقدمك يومًا بيوم.'), style: TextStyle(color: SN.textSec, height: 1.35))])),
              ]),
          ...
          const SizedBox(height: 8),
          GlassCard(child: ListTile(leading: Icon(Icons.emoji_events_rounded, color: SN.gold), title: Text('الأوسمة الرياضية', style: TextStyle(fontWeight: FontWeight.w900)), subtitle: Text('النشيط • بطل التحدي • روح الفريق'), trailing: Icon(Icons.chevron_left_rounded))),
          const SizedBox(height: 10),
          GlassCard(child: ListTile(leading: Icon(Icons.groups_rounded, color: SN.violet), title: Text('مجموعات التحدي', style: TextStyle(fontWeight: FontWeight.w900)), subtitle: Text('أنشئ تحديًا مع أصدقائك واجمعوا النقاط معًا'), trailing: Icon(Icons.chevron_left_rounded))),
          const SizedBox(height: 10),
          GlassCard(child: ListTile(leading: Icon(Icons.privacy_tip_outlined, color: SN.cyan), title: Text('الخصوصية', style: TextStyle(fontWeight: FontWeight.w900)), subtitle: Text('المشاركة في الترتيب اختيارية، ولا تتم مشاركة بيانات الصحة دون إذنك.'))),
        ],
      ),
    );
  }
  ...
  Widget _stat(String title, String value, IconData icon) => Container(
    ...
    child: Row(children: [Icon(icon, color: SN.cyan), SizedBox(width: 8), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: TextStyle(color: SN.textSec, fontSize: 12)), Text(value, style: TextStyle(fontSize: 17, fontWeight: FontWeight.w900))]))]),
  );
      ...
      child: GlassCard(
        ...
        child: Column(children: [
          Row(children: [
            ...
            const SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(c.title, style: TextStyle(fontWeight: FontWeight.w900, fontSize: 16)), SizedBox(height: 3), Text(c.description, style: TextStyle(color: SN.textSec, fontSize: 12, height: 1.3))])),
