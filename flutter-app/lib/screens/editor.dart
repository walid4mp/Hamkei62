      ...
      builder: (ctx, setLocal) {
        ...
        void publish() async {
          ...
          try {
            ...
            final publishSettings = await ContentStudio.showPrePublish(
              ctx,
              type: reel ? 'reel' : 'post',
              initial: {'visibility': 'PUBLIC', 'commentsEnabled': true, 'repostEnabled': true},
            );
            if (publishSettings == null) return;
            if (publishSettings['saveDraft'] == true) {
              await ContentStudio.saveDraft({'type': reel ? 'reel' : 'post', 'mediaPath': mediaPath, 'mediaType': mediaType, 'caption': cap, 'title': title.text.trim(), 'musicTitle': music.text.trim(), 'musicUrl': musicUrl, 'rotationDegrees': rotation, 'overlayText': overlayText.text.trim(), 'overlayEmoji': emoji, 'overlayImageUrl': overlayImageUrl});
              if (ctx.mounted) { Navigator.pop(ctx, {'draft': true, 'caption': cap}); toast(context, 'تم حفظ المسودة على الجهاز 📝'); }
              return;
            }
            final payload = {
              'url': url,
            ...
            if (existingId != null) {
              ...
              if (reel) {
                await Api.createReel(url, cap, music.text.trim(), musicUrl: musicUrl, title: title.text.trim(), rotationDegrees: rotation, overlayText: overlayText.text.trim(), overlayEmoji: emoji, overlayImageUrl: overlayImageUrl, commentsEnabled: publishSettings['commentsEnabled'] != false, repostEnabled: publishSettings['repostEnabled'] != false, archived: publishSettings['archived'] == true, scheduledAt: publishSettings['scheduledAt'] as String?);
              } else {
                await Api.post(cap, media: url, type: mediaType, visibility: '${publishSettings['visibility'] ?? 'PUBLIC'}', title: title.text.trim(), rotationDegrees: rotation, overlayText: overlayText.text.trim(), overlayEmoji: emoji, commentsEnabled: publishSettings['commentsEnabled'] != false, repostEnabled: publishSettings['repostEnabled'] != false, pinned: publishSettings['pinned'] == true, archived: publishSettings['archived'] == true, scheduledAt: publishSettings['scheduledAt'] as String?);
              }
            }
          ...
          child: Padding(
            ...
            child: SingleChildScrollView(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  Expanded(child: Text(L10n.t('تعديل قبل النشر'), style: TextStyle(fontSize: 19, fontWeight: FontWeight.w800))),
