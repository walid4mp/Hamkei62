          ...
          child: SizedBox(
            ...
            child: Column(children: [
              Padding(padding: const EdgeInsets.fromLTRB(18, 8, 18, 10), child: Row(children: [
                ...
                Text('${selected.length} محدد', style: const TextStyle(color: SN.textMut, fontSize: 12)),
              ])),
              ...
              Padding(padding: const EdgeInsets.all(14), child: SizedBox(width: double.infinity, child: FilledButton.icon(
                ...
                icon: const Icon(Icons.send_rounded), label: const Text('إرسال المشاركة'),
              ))),
            ]),
          ),
        ),
              ...
              children: [
                if (results.isNotEmpty)
                  const Padding(
                    ...
                    child: Text('أشخاص لبدء محادثة',
                        style: TextStyle(color: SN.cyan, fontWeight: FontWeight.w800, fontSize: 13)),
                  ),
...
class _ChatPageState extends State<ChatPage> {
  ...
  Future<void> _load() async {
    try {
      ...
      setState(() {
        msgs
          ..clear()
          ..addAll(r.map((e) => Map<String, dynamic>.from(e as Map)));
        loading = false;
      });
      _scrollDown();
    } catch (e) {
  ...
  void _chatSettings() {
    showModalBottomSheet<void>(
      ...
      builder: (ctx) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [
        const ListTile(title: Text('إعدادات المحادثة', style: TextStyle(fontWeight: FontWeight.w900))),
  ...
  Future<void> _secretSettings() async {
    ...
    final result = await showModalBottomSheet<Map<String,dynamic>>(
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setSheet) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [
        const ListTile(leading: Icon(Icons.shield_rounded, color: SN.violet), title: Text('الوضع السري', style: TextStyle(fontWeight: FontWeight.w900)), subtitle: Text('محادثة مؤقتة مع حماية إضافية للخصوصية')),
  ...
  void _moreActions() {
    showModalBottomSheet<void>(
      ...
      builder: (ctx) => SafeArea(
        child: Wrap(
          children: [
            ListTile(leading: const Icon(Icons.photo_library_outlined, color: SN.violet), title: const Text('إرسال صورة من المعرض'), onTap: () { Navigator.pop(ctx); _sendMedia(source: ImageSource.gallery); }),
            ListTile(leading: const Icon(Icons.camera_alt_outlined, color: SN.violet), title: const Text('التقاط صورة'), onTap: () { Navigator.pop(ctx); _sendMedia(source: ImageSource.camera); }),
            ListTile(leading: const Icon(Icons.videocam_outlined, color: SN.violet), title: const Text('إرسال فيديو'), onTap: () { Navigator.pop(ctx); _sendVideo(); }),
            ListTile(leading: const Icon(Icons.card_giftcard_outlined, color: SN.cyan), title: const Text('إرسال هدية'), onTap: () { Navigator.pop(ctx); showGiftPicker(context, receiverId: widget.userId, receiverName: widget.name, contextType: 'MESSAGE'); }),
          ],
        ),
      ),
    );
  ...
  void dispose() {
    ...
    ChatBubbleManager.i.show(userId: widget.userId, name: widget.name, avatar: widget.avatar, onOpen: () {
      final nav = _navigator;
      if (nav != null && nav.mounted) {
        nav.push(MaterialPageRoute(builder: (_) => ChatPage(userId: widget.userId, name: widget.name, avatar: widget.avatar)));
      }
    });
    super.dispose();
  }
  ...
  Widget _messageContent(String body, bool mine, String messageId) {
    ...
    if (body.startsWith('[group_share]')) {
      ...
      return InkWell(
        ...
        child: Container(
          ...
          child: Row(children: [
            Container(width:46,height:46,decoration:BoxDecoration(gradient:SN.grad,borderRadius:BorderRadius.circular(14)),child:const Icon(Icons.groups_rounded,color:Colors.white)),
            const SizedBox(width:10),
            Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
              const Text('مشاركة مجموعة',style:TextStyle(fontSize:11,color:SN.textMut)),
              Text(name,maxLines:2,overflow:TextOverflow.ellipsis,style:const TextStyle(fontWeight:FontWeight.w800)),
              const SizedBox(height:4),
              const Text('اضغط للفتح',style:TextStyle(fontSize:11,color:SN.violet)),
            ])),
          ]),
        ),
      );
    ...
    if (body.startsWith('[live_share]')) {
      ...
      return InkWell(
        ...
        child: Container(width:240,padding:const EdgeInsets.all(13),decoration:BoxDecoration(color:mine?Colors.white.withValues(alpha:.12):SN.bg3,borderRadius:BorderRadius.circular(17)),child:Row(children:[
          Container(width:46,height:46,decoration:BoxDecoration(color:SN.red,borderRadius:BorderRadius.circular(14)),child:const Icon(Icons.sensors_rounded,color:Colors.white)),
          const SizedBox(width:10),
          Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[const Text('مشاركة Live',style:TextStyle(fontSize:11,color:SN.textMut)),Text(title,maxLines:2,overflow:TextOverflow.ellipsis,style:const TextStyle(fontWeight:FontWeight.w800)),const SizedBox(height:4),const Text('اضغط للمشاهدة',style:TextStyle(fontSize:11,color:SN.red))])),
        ])),
      );
    }
    if (body.startsWith('[image]')) {
      final url = body.substring(7);
      if(messageId.isNotEmpty && _viewedMediaMessages.add(messageId)) Future.microtask(()=>Api.viewMessage(messageId)); return ClipRRect(borderRadius: BorderRadius.circular(14), child: Image.network(url, width: 220, height: 220, fit: BoxFit.cover, errorBuilder: (_, __, ___) => const Padding(padding: EdgeInsets.all(10), child: Text('تعذّر تحميل الصورة'))));
    }
    ...
    if (body.startsWith('[audio]')) {
      ...
      return InkWell(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.play_circle_fill_rounded, color: mine ? Colors.white : SN.violet, size: 34), const SizedBox(width: 8), Text('رسالة صوتية', style: TextStyle(color: mine ? Colors.white : SN.textPri, fontWeight: FontWeight.w700))]),
      );
    }
    return Text(body, style: const TextStyle(height: 1.5, fontSize: 14));
  }
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          Padding(padding: const EdgeInsets.only(right: 6), child: CircleAvatar(radius: 18, backgroundColor: SN.bg3, child: IconButton(padding: EdgeInsets.zero, onPressed: () => Navigator.pop(context), icon: const Icon(Icons.close_rounded, size: 20)))),
        ],
      ),
      body: Column(
        children: [
                            ...
                            child: GestureDetector(
                              ...
                              child: Container(
                                ...
                                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                  if (msgs[i]['secret'] == true) Padding(padding: const EdgeInsets.only(bottom: 4), child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.shield_rounded, size: 13, color: mine ? Colors.white70 : SN.violet), const SizedBox(width: 4), Text('سري • مؤقت', style: TextStyle(fontSize: 10, color: mine ? Colors.white70 : SN.textMut, fontWeight: FontWeight.w700))])),
          ...
          if (secretMode) Padding(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 3), child: Row(children: [const Icon(Icons.shield_rounded, size: 15, color: SN.violet), const SizedBox(width: 5), Text('الوضع السري مفعّل • ${secretTtl == 1440 ? '24 ساعة' : '$secretTtl دقيقة'}', style: const TextStyle(fontSize: 11, color: SN.violet, fontWeight: FontWeight.w800)), const Spacer(), if(selfDestruct) const Icon(Icons.auto_delete_rounded, size: 15, color: SN.violet)])),
          Container(
            ...
            child: Directionality(
              ...
              child: Row(
                children: [
                  Container(
                    width: 56, height: 56,
                    decoration: const BoxDecoration(gradient: SN.grad, shape: BoxShape.circle),
                  ...
                  Expanded(
                    child: recording
                        ? const Padding(padding: EdgeInsets.symmetric(horizontal: 10), child: Text('جاري تسجيل رسالة صوتية… اضغط الميكروفون للإرسال', style: TextStyle(color: Colors.black54, fontSize: 13)))
                        : TextField(
                            controller: ctrl,
                            onSubmitted: (_) => _send(),
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: SingleChildScrollView(
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            DropdownButtonFormField<String>(value: privacy, decoration: const InputDecoration(labelText: 'الخصوصية', prefixIcon: Icon(Icons.lock_outline_rounded)), items: const [DropdownMenuItem(value: 'PUBLIC', child: Text('عامة — يمكن للجميع الانضمام')), DropdownMenuItem(value: 'PRIVATE', child: Text('خاصة — للأعضاء فقط'))], onChanged: (v) => privacy = v ?? 'PUBLIC'),
            const SizedBox(height: 10),
                    ...
                    children: [
                      Container(
                        ...
                        child: const Row(children: [
                          CircleAvatar(backgroundColor: Colors.white24, child: Icon(Icons.forum_rounded, color: Colors.white)),
                          SizedBox(width: 12),
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('مجتمعات SocialNova', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16)), SizedBox(height: 4), Text('اكتشف مجموعات جديدة، شارك، وتواصل مع الأعضاء.', style: TextStyle(color: Colors.white70, fontSize: 12))])),
                        ]),
                      ),
    ...
    return GlassCard(
      ...
      child: InkWell(
        ...
        child: Padding(padding: const EdgeInsets.all(14), child: Row(children: [
          Container(width: 60, height: 60, decoration: BoxDecoration(borderRadius: BorderRadius.circular(18), gradient: SN.grad, image: avatar.isEmpty ? null : DecorationImage(image: NetworkImage(avatar), fit: BoxFit.cover)), child: avatar.isEmpty ? const Icon(Icons.groups_rounded, color: Colors.white, size: 28) : null),
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [Expanded(child: Text('${group['name']}', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 15))), Icon(private ? Icons.lock_rounded : Icons.public_rounded, size: 16, color: SN.textMut)]),
            const SizedBox(height: 5),
            Text('${members} عضو • ${private ? 'خاصة' : 'عامة'}', style: const TextStyle(color: SN.textMut, fontSize: 12)),
            if ('${group['description'] ?? ''}'.isNotEmpty) ...[const SizedBox(height: 3), Text('${group['description']}', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: SN.textSec, fontSize: 12))],
          ])),
...
class _GroupChatPageState extends State<GroupChatPage> {
  ...
  Future<void> _editGroup() async {
    ...
    final ok=await showDialog<bool>(context: context,builder:(ctx)=>AlertDialog(title:const Text('إعدادات المجموعة'),content:SingleChildScrollView(child:Column(mainAxisSize:MainAxisSize.min,children:[TextField(controller:name,decoration:const InputDecoration(labelText:'اسم المجموعة')),const SizedBox(height:10),TextField(controller:desc,maxLines:3,decoration:const InputDecoration(labelText:'الوصف')),const SizedBox(height:10),DropdownButtonFormField<String>(value:privacy,decoration:const InputDecoration(labelText:'الخصوصية'),items:const[DropdownMenuItem(value:'PUBLIC',child:Text('عامة')),DropdownMenuItem(value:'PRIVATE',child:Text('خاصة'))],onChanged:(v)=>privacy=v??privacy),const SizedBox(height:10),TextField(controller:rules,maxLines:4,decoration:const InputDecoration(labelText:'قواعد المجموعة'))])),actions:[TextButton(onPressed:()=>Navigator.pop(ctx,false),child:const Text('إلغاء')),FilledButton(onPressed:()=>Navigator.pop(ctx,true),child:const Text('حفظ'))]));
  ...
  void _showGroupSettings() {
    showModalBottomSheet(context:context,isScrollControlled:true,showDragHandle:true,backgroundColor:SN.bg1,shape:const RoundedRectangleBorder(borderRadius:BorderRadius.vertical(top:Radius.circular(28))),builder:(ctx)=>SafeArea(child:Padding(padding:const EdgeInsets.fromLTRB(16,4,16,20),child:Column(mainAxisSize:MainAxisSize.min,crossAxisAlignment:CrossAxisAlignment.stretch,children:[
      Row(children:[Container(width:48,height:48,decoration:BoxDecoration(borderRadius:BorderRadius.circular(15),gradient:SN.grad),child:const Icon(Icons.groups_rounded,color:Colors.white)),const SizedBox(width:12),Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text('${groupInfo['name']??widget.name}',style:const TextStyle(fontSize:18,fontWeight:FontWeight.w800)),Text('${((groupInfo['_count']??{}) as Map)['members']??0} عضو',style:const TextStyle(color:SN.textMut,fontSize:12))]))]),
      const SizedBox(height:14),
      ListTile(leading:const Icon(Icons.share_rounded,color:SN.violet),title:const Text('مشاركة المجموعة في المحادثة'),subtitle:const Text('أرسل بطاقة المجموعة إلى أصدقائك'),onTap:(){Navigator.pop(ctx);shareSocialItemToChats(context,title:'مشاركة المجموعة',body:'[group_share]${widget.groupId}|${groupInfo['name'] ?? widget.name}',icon:Icons.groups_rounded);}),
  ...
  @override Widget build(BuildContext context){
    final myId='${Api.me?['id']}';
    return Scaffold(
      appBar: AppBar(backgroundColor:SN.bg1,titleSpacing:4,title:InkWell(onTap:_showGroupSettings,child:Row(children:[Container(width:38,height:38,decoration:BoxDecoration(borderRadius:BorderRadius.circular(12),gradient:SN.grad),child:const Icon(Icons.groups_rounded,color:Colors.white)),const SizedBox(width:9),Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text('${groupInfo['name']??widget.name}',maxLines:1,overflow:TextOverflow.ellipsis,style:const TextStyle(fontSize:15,fontWeight:FontWeight.w800)),Text('${((groupInfo['_count']??{}) as Map)['members']??0} عضو',style:const TextStyle(fontSize:10,color:SN.textMut))]))])),actions:[IconButton(tooltip:'بحث',onPressed:()=>toast(context,'بحث الرسائل سيكون متاحاً قريباً'),icon:const Icon(Icons.search_rounded)),IconButton(tooltip:'إعدادات المجموعة',onPressed:_showGroupSettings,icon:const Icon(Icons.more_vert_rounded))]),
      body:loading?const LoadingBox():Column(children:[
        if(!joined) Container(margin:const EdgeInsets.all(12),padding:const EdgeInsets.all(14),decoration:BoxDecoration(gradient:SN.grad,borderRadius:BorderRadius.circular(18)),child:Row(children:[const Expanded(child:Text('هذه المجموعة خاصة بك. انضم لرؤية المحادثة.',style:TextStyle(fontWeight:FontWeight.w700))),FilledButton(onPressed:_toggleJoin,child:const Text('انضمام'))])),
        Expanded(child:msgs.isEmpty?const EmptyState(text:'لا توجد رسائل بعد\nابدأ أول محادثة في المجموعة',icon:Icons.forum_outlined):ListView.builder(controller:scroll,padding:const EdgeInsets.fromLTRB(14,12,14,18),itemCount:msgs.length,itemBuilder:(c,i){final mine='${msgs[i]['senderId']}'==myId;final sender=Map<String,dynamic>.from((msgs[i]['sender']??{}) as Map);return Align(alignment:mine?Alignment.centerLeft:Alignment.centerRight,child:Column(crossAxisAlignment:mine?CrossAxisAlignment.end:CrossAxisAlignment.start,children:[if(!mine)Padding(padding:const EdgeInsets.only(bottom:2,right:8),child:Text('${sender['displayName']??sender['username']??''}',style:const TextStyle(color:SN.textMut,fontSize:11,fontWeight:FontWeight.w600))),Container(margin:EdgeInsets.symmetric(vertical:compactMode?2:5),padding:EdgeInsets.symmetric(horizontal:compactMode?11:14,vertical:compactMode?7:10),constraints:BoxConstraints(maxWidth:MediaQuery.of(context).size.width*.78),decoration:BoxDecoration(gradient:mine?SN.grad:null,color:mine?null:SN.bg2,borderRadius:BorderRadius.only(topLeft:const Radius.circular(18),topRight:const Radius.circular(18),bottomLeft:Radius.circular(mine?18:4),bottomRight:Radius.circular(mine?4:18)),border:mine?null:Border.all(color:SN.strokeSoft)),child:Text('${msgs[i]['body']??''}',style:TextStyle(height:1.45,fontSize:compactMode?13:14))) ]));})),
        if(joined)Container(padding:const EdgeInsets.fromLTRB(10,8,10,10),decoration:const BoxDecoration(color:SN.bg1,border:Border(top:BorderSide(color:SN.strokeSoft))),child:Row(children:[IconButton(onPressed:()=>toast(context,'إرسال الصور والملفات سيكون متاحاً قريباً'),icon:const Icon(Icons.add_circle_outline_rounded,color:SN.violet)),Expanded(child:TextField(controller:ctrl,onSubmitted:(_)=>_send(),textInputAction:TextInputAction.send,decoration:const InputDecoration(hintText:'اكتب رسالة...',contentPadding:EdgeInsets.symmetric(horizontal:14,vertical:11))),),const SizedBox(width:7),Container(decoration:BoxDecoration(gradient:SN.grad,shape:BoxShape.circle),child:IconButton(onPressed:_send,icon:const Icon(Icons.send_rounded,color:Colors.white))) ])),
      ]),
    );
  }
}
...
class _CallRoomPageState extends State<CallRoomPage> {
  ...
  bool connected=false, micOn=true, cameraOn=false;
  String? error;
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Row(children: [Expanded(child: Text('${r['host']?['displayName'] ?? ''} • ${r['viewerCount'] ?? 0} مشاهد', style: const TextStyle(color: SN.textMut, fontSize: 12))), if (r['demoViewers'] == true) const Text('DEMO', style: TextStyle(color: SN.cyan, fontSize: 9, fontWeight: FontWeight.w800))]),
                                    ],
                                  ),
                                ),
                                FilledButton(
              ...
              children: [
                ...
                Expanded(
                  child: Container(
                    ...
                    child: Row(
                      children: [
                        const Icon(Icons.sensors_rounded, color: SN.pink, size: 17),
                        const SizedBox(width: 7),
                        Expanded(
              ...
              child: ColoredBox(
                ...
                child: Center(
                  child: Column(mainAxisSize: MainAxisSize.min, children: [
                    ...
                    SizedBox(height: 14),
                    Text('جاري فتح البث بجودة عالية...', style: TextStyle(color: Colors.white)),
                  ]),
                ),
              ),
            ),
